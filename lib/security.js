...
const ALLOWED_ROOF_AGES = new Set([
    'Under 10 years',
    '10 - 20 years',
    '20+ years',
    'Not sure',
...
export function slugFromHost(rawHost) {
    const forced = String(process.env.CLIENT_SLUG || '').trim().toLowerCase();
    if (forced) {
        return forced.replace(/[^a-z0-9-]/g, '').slice(0, 40) || 'default';
    }

    const host = String(rawHost || '').split(',')[0].trim().toLowerCase().split(':')[0];
    if (!host) return 'unknown';
    if (host === 'localhost' || host.startsWith('127.') || host.endsWith('.local')) return 'local';

    if (host === 'quotramax.com' || host === 'www.quotramax.com') return 'quotramax';
    if (host.endsWith('.quotramax.com')) {
        const sub = host.slice(0, -'.quotramax.com'.length);
        if (sub && sub !== 'www') return sub.replace(/[^a-z0-9-]/g, '').slice(0, 40) || 'quotramax';
        return 'quotramax';
    }

    const first = host.split('.')[0].replace(/[^a-z0-9-]/g, '').slice(0, 40);
    return first || 'unknown';
}

export function getClientSlug(req) {
    return slugFromHost(req.headers.get('x-forwarded-host') || req.headers.get('host') || '');
}
...
export function validateLeadPayload(body) {
    ...
    return {
        value: {
            ...
            roofAge: oneOf(body.roofAge, ALLOWED_ROOF_AGES, '10 - 20 years'),
