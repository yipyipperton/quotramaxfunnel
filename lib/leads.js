...
export function mapLead(lead) {
    ...
    return {
        ...
        date: lead.date,
        client: lead.client || extraData.client || '',
        service: extraData.service || lead.age || 'Full Roof Replacement',
        timeline: extraData.timeline || '1 - 4 weeks',
...
export async function insertLead(leadData) {
    ...
    if (supabase) {
        ...
        for (const row of attempts) {
            ...
            if (/column .*client/i.test(combined) && row.client) {
                const { client: _ignored, ...withoutClient } = row;
                const retry = await supabase.from('leads').insert([withoutClient]).select('id').maybeSingle();
                if (!retry.error) {
                    return { ...withoutClient, ...(retry.data || {}) };
                }
            }
        }
...
export async function getContractorEmail(slug) {
    const config = slug ? await getClientConfig(slug) : null;
    if (config?.contractorEmail) return config.contractorEmail;
