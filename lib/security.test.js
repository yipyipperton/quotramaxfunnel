...
import { getFromAddress } from './email.js';

describe('getFromAddress', () => {
    it('defaults to inspections@quotramax.com', () => {
        const prev = process.env.FROM_EMAIL;
        delete process.env.FROM_EMAIL;
        try {
            assert.equal(getFromAddress(), 'Quotramax Inspections <inspections@quotramax.com>');
        } finally {
            if (prev === undefined) delete process.env.FROM_EMAIL;
            else process.env.FROM_EMAIL = prev;
        }
    });

    it('rewrites the old leads@ alias if it is still in env', () => {
        const prev = process.env.FROM_EMAIL;
        process.env.FROM_EMAIL = 'Quotramax Inspection Team <leads@quotramax.com>';
        try {
            assert.equal(getFromAddress(), 'Quotramax Inspections <inspections@quotramax.com>');
        } finally {
            if (prev === undefined) delete process.env.FROM_EMAIL;
...
describe('formatPhone', () => {
    it('formats the stored 10-digit value for display', () => {
        assert.equal(formatPhone('8135550147'), '(813) 555-0147');
    });

    it('drops a leading country code', () => {
        assert.equal(formatPhone('18135550147'), '(813) 555-0147');
    });

    it('leaves already formatted and unexpected values readable', () => {
        assert.equal(formatPhone('(813) 555-0147'), '(813) 555-0147');
        assert.equal(formatPhone('555'), '555');
        assert.equal(formatPhone(''), '');
    });

    it('builds a dialable tel href', () => {
        assert.equal(telHref('(813) 555-0147'), '+18135550147');
    });
});
...
describe('validateLeadPayload', () => {
    ...
    it('accepts a complete valid payload', () => {
        ...
        assert.equal(result.value.pitch, 'Steep Pitch');
        assert.equal(result.value.roofAge, '20+ years');
    });

    it('falls back to standard pitch and mid-age roof when omitted', () => {
        const result = validateLeadPayload({ ...valid, pitch: undefined, roofAge: undefined });
        assert.equal(result.error, undefined);
        assert.equal(result.value.pitch, 'Standard Pitch');
        assert.equal(result.value.roofAge, '10 - 20 years');
    });
